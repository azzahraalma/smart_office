...
if ($absenHariIni && !empty($absenHariIni['jam_masuk'])) {

    $tsMasuk   = strtotime($absenHariIni['jam_masuk']);
    $sekarang  = time();

    // total waktu dari masuk sampai sekarang
    $totalDetik = $sekarang - $tsMasuk;

    // total break (dalam menit) → kamu sudah punya $totalBreakMnt
...
$batas = $absenHariIni 
    ? strtotime($absenHariIni['jam_masuk']) + (8 * 3600)
    : null;
$sekarang = time();
?>
        ...
        <div class="stat-card success">
            ...
            <div class="stat-card-value" style="color:var(--success)">
                <?php if ($absenHariIni && !$absenHariIni['jam_keluar']): ?>
                    <?php $sisa = max(0, $batas - $sekarang); echo floor($sisa / 3600) . 'j ' . floor(($sisa % 3600) / 60) . 'm'; ?>
                <?php else: ?>
                    -
                <?php endif; ?>
            </div>
            <div class="stat-card-sub">
                <?= $absenHariIni ? 'Selesai pukul ' . date('H:i', $batas) : '-' ?>
            </div>
        </div>
    </div>
    <div class="col-6 col-lg-3">
...
<div class="overtime-card mb-4 <?= $overtime ? 'ot-active' : 'ot-normal' ?>">
    <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <div style="flex-shrink:0;">
            <a href="/absen/pulang" class="btn-so-primary" style="font-size:13px;padding:8px 18px;white-space:nowrap;gap:6px;">
                ...
                <div class="timeline-list">
                    ...
                    <div class="timeline-item">
                        ...
                        <div class="timeline-content">
                            ...
                            <div class="timeline-sub">Sudah melewati jam 17:00</div>
                        </div>
                    </div>
...
/* ── Overtime Card ── */
.overtime-card        { padding:18px 24px;border-radius:var(--radius);border:1.5px solid; }
.ot-normal            { background:linear-gradient(135deg,#eff6ff,#dbeafe);border-color:#93c5fd; }
.ot-active            { background:linear-gradient(135deg,#fee2e2,#fecaca);border-color:#f87171;animation:pulse-border 2s infinite; }
.ot-icon              { width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0; }
.ot-normal .ot-icon   { background:#dbeafe;color:#1d4ed8; }
.ot-active .ot-icon   { background:#fecaca;color:#dc2626; }
.ot-label             { font-size:15px;font-weight:800;color:var(--text);margin-bottom:2px; }
.ot-sub               { font-size:12px;color:var(--text-muted);line-height:1.6; }
.ot-progress-wrap     { min-width:180px;flex-shrink:0; }
.ot-bar               { height:6px;background:#bfdbfe;border-radius:999px;overflow:hidden; }
.ot-fill              { height:100%;background:#1d4ed8;border-radius:999px;transition:width .4s; }
...
.timeline-icon  { width:36px;height:36px;border-radius:50%;background:var(--border);color:var(--text-muted);display:flex;align-items:center;justify-content:center;flex-shrink:0; }
